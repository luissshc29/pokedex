...
export default function Cards() {
  ...
  const [listaPokemonDetalhada, setListaPokemonDetalhada] = useState<
    IPokemon[]
  >([]);

  const { busca, aleatorizaLista, alteraBusca, alteraFiltro } =
    useFiltrosContext();
      ...
      listaBuscada = listaBuscada.filter((item) => {
        if (busca.toLowerCase().includes("mega ")) {
          ...
          return item.name.includes(
            buscaFormatada.toLowerCase().replaceAll(" ", "-")
          );
          ...
          busca.toLowerCase().includes("gigantamax") ||
          busca.toLowerCase().includes("gmax")
        ) {
          var buscaFormatada = busca
            .toLowerCase()
            .replace("gigantamax", "")
  ...
  return (
    <>
      <div className={styles.cards}>
        {listaPokemonDetalhada.length > 0 ? (
          <h2 className={styles.mensagem}>
            Exibindo todos os resultados{" "}
            {aleatoria ? <span>aleatórios</span> : ""}{" "}
            {busca ? `para "${busca}"` : ""}{" "}
            {filtro ? (
              <i>
                do tipo <span>{filtro}</span>
              </i>
            ) : (
              ""
            )}{" "}
        ...
        {ordenaLista(listaPokemonDetalhada).map((item) => {
          if (item) {
            return <Card key={uuidv4()} pokemon={item} />;
          }
        })}
      </div>
    </>
